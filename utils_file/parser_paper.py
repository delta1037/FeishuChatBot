...
class Paper:
    ...
    def _get_all_page_index(self):
        ...
        for page_index, page in enumerate(self.pdf):
            ...
            # print(page_index, cur_text)
            # 遍历需要寻找的章节名称列表
            for section_name in section_list:
                # 将章节名称转换成大写形式
                section_name_upper = section_name.upper()
                # 如果当前页面包含"Abstract"这个关键词
                if "Abstract" == section_name and section_name in cur_text and section_name not in section_page_dict:
...
# 将paper封装成可以给ChatGPT阅读的方式
def parse_paper(paper_path, title=''):
    ...
    if 'Abstract' in paper.section_text_dict:
        text += '\nAbstract: ' + paper.section_text_dict['Abstract']
    if 'Introduction' in paper.section_text_dict:
        text += '\nIntroduction: ' + paper.section_text_dict['Introduction']
    if 'Conclusions' in paper.section_text_dict:
        text += '\nConclusion: ' + paper.section_text_dict['Conclusions']
    if 'Conclusion' in paper.section_text_dict:
        text += '\nConclusion: ' + paper.section_text_dict['Conclusion']
    # intro_title = next((item for item in paper.section_names if 'ntroduction' in item.lower()), None)
    # if intro_title is not None:
    #     text += '\nIntroduction: ' + paper.section_text_dict[intro_title]
    ...
    #     text += '\nConclusion: ' + paper.section_text_dict[conclusion_title]
    #
    return text


def main():
    print(parse_paper('../demo_en_1.pdf'))
