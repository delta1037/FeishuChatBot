...
class DeepSeekGptApi:
    ...
    # 对话生成接口
    @staticmethod
    def chat_gen(prompt, system=None, history=None, model_type='deepseek-chat', max_tokens=config['max_chars']):
        gpt_result = {
            'answer': '',
            'prompt_tokens': 0,
            'completion_tokens': 0,
        ...
        try:
            # tokens = DeepSeekGptApi.num_tokens_from_string(prompt, model_type)
            # if tokens > max_tokens:
            #     gpt_result['answer'] = '你的输入太长了，我理解不了 😭'
            #     return gpt_result
            completion = openai.ChatCompletion.create(
                api_key=config['deep_seek_api_keys'][0],
